  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mx-5 mt-5">
      {templateList.map((template) => (
        <div key={template.slug} className="hover:bg-green-400">
          <Link
            href={`/dashboard/${template.slug}`}
            className="bg-white w-full rounded-xl h-[200px] py-4 px-4 text-center flex flex-col justify-center hover:bg-slate-100"
          >
